##########################################################################
##########################################################################
#' This file is the equivalent of custom_functions.R file from Franz. 
#' I decided to change the name so it is more intuitive for me. 
#' 
#' You will be able to notice that I copied many on Franz's and Giussppe's 
#' code with minor mofifications. 
#' 
#' I also added extra functions for additional QC analysis and 
#' alternative loading of Seurat objects. 
##########################################################################
##########################################################################


#' Read CosMx Spatial Transcriptomics Data
#' Code from Franz and Giuseppe
#'
...
fov_plotting  <- function(experiment_repo, obj) {
    ...
    plot_transcript <- lapply(seq_along(slide_paths), function(i) {
        ...
        if (length(fov_file) == 0) stop("No FOV position file found for: ", slidename)

        fov_pos <- data.table::fread(file.path(current_path, fov_file), showProgress = FALSE)
        
        mean_transcript <- subset(meta.data, slidename == slidename) %>% 
        ...
        plot <- ggplot2::ggplot(fov_pos, aes(x = fov_pos[[x_col]], y = fov_pos[[y_col]], fill = mean_transcript)) +
          ...
          labs(title = paste0("Slide: ", slidename))
    })
